...
from src.notes.keyboards.inline import get_delete_note_inline_kb
from src.notes.keyboards.reply import NotesButtons
from src.notes.states import DeleteNoteState
...
async def delete_note_with_name(message: Message, state: FSMContext, arg: str):
    await service.delete_note_state_name(name=arg, message=message, state=state)
...
async def delete_note_state_name(message: Message, state: FSMContext) -> None:
    await service.delete_note_state_name(name=message.text, message=message, state=state)
